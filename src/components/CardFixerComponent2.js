...
export default ({ data, handleInfoModa }) => {
  ...
  let curr_time = moment()
    .utc(new Date(minute20))
    .local()
    .format('YYYY-MM-DD HH:mm');
  let start_time = moment()
    .utc(new Date(data.appointment_at))
    .local()
    .format('YYYY-MM-DD HH:mm');
  ...
  const coinIcon = (
    <Icon name="coins" type="font-awesome-5" color={black} size={20} />
  );
  const calendarIcon = (
    <Icon name="calendar" type="font-awesome-5" color={black} size={20} solid />
  );
  const clockIcon = (
    <Icon name="clock" type="font-awesome-5" color={black} size={20} solid />
  );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={[styles.textContainer, { top: 6 }]}>
          ...
          <Text style={styles.subline}>{data.street_address}</Text>
        </View>
      </TouchableOpacity>
      <TouchableOpacity
        style={styles.addressContainer}
        onPress={() => Linking.openURL('tel:' + data.customer.phone_no)}
      >
        <Icon
          name="mobile-alt"
          type="font-awesome-5"
        ...
        <View style={styles.textContainer}>
          <Text style={styles.line1}>{data.customer.phone_no}</Text>
        </View>
      </TouchableOpacity>
        ...
        <View style={{ width: '70%' }}>
          ...
          <View style={{ marginVertical: 5 }}>
            <Text style={styles.locStyle}>
              {' '}
              Portkod: {data.place}, {data.port_code}
            </Text>
          </View>

          <View style={{ marginVertical: 5, width: '80%' }}>
            <Text style={styles.sublineItalic}>{data.notes}</Text>
          </View>
          {data.service.icon == 'dot-circle' ||
          data.service.icon == 'car-side' ? (
            <View style={{ width: '100%' }}>
              ...
              <View style={{ marginTop: 10, flexDirection: 'row' }}>
                ...
                <View style={{ width: '50%' }}>
                  <Text style={styles.carInfo2}>
                    {data.car_brand ? data.car_brand : 'Not available'}
                  </Text>
                </View>
              </View>
              ...
              <View style={{ marginTop: 10, flexDirection: 'row' }}>
                ...
                <View style={{ width: '50%' }}>
                  <Text style={styles.carInfo2}>
                    {data.model ? data.model : 'Not available'}
                  </Text>
                </View>
              </View>
              ...
              <View style={{ marginTop: 10, flexDirection: 'row' }}>
                ...
                <View style={{ width: '50%' }}>
                  <Text style={styles.carInfo2}>
                    {data.color ? data.color : 'Not available'}
                  </Text>
                </View>
              </View>
            </View>
