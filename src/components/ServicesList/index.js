...
export default ({ services, countCartItems }) => {
    ...
    <ScrollView>
      ...
      {services.map((item) => {
        ...
        if (user.organization_number == null) {
          let exact_price = 0;
          if (
            item.title === 'Däckbyte hos dig' ||
            item.title === 'Biltvätten hos dig'
          ) {
            exact_price = item.prices[0];
          } else {
            exact_price = parseFloat(item.prices[0]) / 2;
          }
          item.title === 'Hänga upp hyllor' ||
          item.title === 'Möbelmontering' ||
          item.title === 'Kratta löv'
