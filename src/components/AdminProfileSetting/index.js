...
export default () => {
  ...
  const setUser = (user) => {
    ...
    setClearingNumber(user.bank_clearing_number);
  };
  ...
  const handleProfile = async () => {
    ...
    formData.append('bank_clearing_number', clearingNumber);
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity style={styles.imgView} onPress={() => selectImage()}>
        {imgHref ? (
          <Image source={{ uri: imgHref }} style={styles.imgStyle} />
        ) : (
          <View>
            <Icon
              name={'user-alt'}
